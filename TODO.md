...
- [ ] **~10% of built structures have no owner** — Observed in multi-bot runs. Diagnostic logging added to ownership transfer patches (blueprint→frame, frame→building). Likely causes: terrain/floor placements that don't go through ThingDef path, or edge cases in the transfer pipeline. Need to analyze logs from next multi-bot run.
